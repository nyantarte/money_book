...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MontlyList(title: 'Flutter Demo Home Page'),
    );
  }
}

class MontlyList extends StatefulWidget {
  const MontlyList({super.key, required this.title});
  ...
  @override
  State<MontlyList> createState() => _MonthlyViewState();
}

class _MonthlyViewState extends State<MontlyList> {
  DateTime beginDateRange=DateTime.now();
  DateTime endDateRange=DateTime.now();

  _MonthlyViewState(){
    var nowDate=DateTime.now();
    beginDateRange=DateTime(nowDate.year,nowDate.month,nowDate.day);
    if(12==nowDate.month){
  ...
  void _addTransaction() {
    setState(() {
      ...
      Navigator.of(context).push(MaterialPageRoute(builder: (context) {
        return EditTransaction(title:"",DateTime.now());
      }));
    });
    void _setDateRange(DateTime begin,DateTime end){
      beginDateRange=begin;
      endDateRange=end;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    dateRangeText="Monthly "+beginDateRange.year.toString()+"/"+beginDateRange.month.toString();
